(function($jq) {
	...
	*/
	...
	$jq.fn.getTwitter = function(options) {

		$jq.fn.getTwitter.defaults = {
			userName: null,
			numTweets: 5,
		...
		var o = $jq.extend({}, $jq.fn.getTwitter.defaults, options);
		...
		return this.each(function() {
			...
			var tl = $jq("#twitter_update_list");

			// hide twitter list
			tl.hide();

			// add preLoader to container element
			var preLoaderHTML = $jq("<p class=\"preLoader\">"+o.loaderText+"</p>");
			...
			$jq.ajax({
				...
				success: function(data) {
					...
					$jq.each(data, function(key, val) {
						...
						$jq("#twitter_update_list").append(tweetHTML);

						count++;

						if (count == o.numTweets) {
							// remove preLoader from container element
							$jq(preLoaderHTML).remove();
